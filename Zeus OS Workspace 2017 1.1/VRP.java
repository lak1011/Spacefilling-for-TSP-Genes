...
public class VRP {
	...
	public VRP(String dataFile) {
		...
		writeShortSolution(dataFile.substring(dataFile.lastIndexOf("/") + 2));
					...
					if (mainDepots.getHead().getNext() == mainDepots.getTail() && mainDepots.getNoDepots() <= 1)
					{
						...
						for (i = 0; i < ProblemInfo.truckTypes.size(); i++) {
							VRPTruckType ttype = (VRPTruckType) ProblemInfo.truckTypes.
									elementAt(i);
							depot.getMainTrucks().insertTruckLast(new VRPTruck(ttype,depot.getXCoord(), depot.getYCoord()));
						}
					}
				} //else
	...
	public void writeLongSolution(String file) {
		try {
			...
			convertTextToExcel(ProblemInfo.
					outputPath + file + "_long.txt");
		}
		catch (IOException ioex) {
			ioex.printStackTrace();
	...
	public void convertTextToExcel(String file)throws IOException {
	    // Sets up the Workbook and gets the 1st (0) sheet.
		System.out.println("IN CONVERSION FOR" + file);
	    File excelFile = new File(ProblemInfo.outputPath + file+ "convert.xlsx");
	    ...
	    // Loop through the files.
	        ...
	        while(scanner.hasNextLine()) {
	            ...
	            XSSFRow tempRow = sheet.getRow(rowNo);
	            if(tempRow == null)
	            ...
	            while(lineScanner.hasNext()) {
	                ...
	                tempRow.getCell(columnNo, XSSFRow.CREATE_NULL_AS_BLANK).setCellValue(output);;
	                columnNo++;
	            }
	            lineScanner.close();
