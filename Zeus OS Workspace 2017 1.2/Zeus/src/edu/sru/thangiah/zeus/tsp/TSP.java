...
public class TSP {
	...
	public int readDataFromExcelFile(String TSPFileName) {
		...
		try {
			type = 0;
			m = 1;   // Number of vehicles
			curRow = sheet.getRow(3); //Current row set to 3 - Dimension
			n = (int)curRow.getCell(1).getNumericCellValue(); //Dimension size
		...
		if (type != 0) { //read in a matrix and output it as it goes along
			rowCounter = 7;
			int[][] distMatrix = new int[n][n]; // Make a matrix number of customers by the same
			for (int i = 0; i < n; i++) { // Loop through all rows
				curRow = sheet.getRow(rowCounter);
				for (int k = 0; k < i; k++) { // Loop through all columns
					distMatrix[i][k] = (int)curRow.getCell(k).getNumericCellValue();
					System.out.print(distMatrix[i][k] + " ");
				}
				System.out.println("");
				rowCounter++;
			}	
		...
		try {
			...
			rowCounter = 6; //set the rowCounter, customer data begin from the 7th row
